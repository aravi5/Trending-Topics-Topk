 ...
 * Modified by Abhishek Ravi (aravi5@ncsu.edu) from original author
...
public class CountMinSketchState implements State {
    ...
    double eps;
    double confidence;
    //aravi5: Modifications to Count-min Sketch to support Top-K
    PriorityQueue<String> heap;		//aravi5: Priority Queue to hold top-k items
    public static final int k = 10; //aravi5: specifies the value of k in top-k
    ...
    public CountMinSketchState(int depth, int width, int seed) {
      ...
      //aravi5: Initialize the heap for Top-k
        this.heap = new PriorityQueue<String>(k, new SketchComparator<String>(this));
    ...
    public void add(String item, long count) {
        ...
        size += count;
        //aravi5: After adding to the count-min, add to the Priority queue that contains
        //		  top-k frequet items. Before adding check if item is in the queue.
        if (!heap.contains(item)) {
        	//aravi5: if the item is not in heap, add it
        	heap.add(item);
        }
        else {
        	//aravi5: if item already exists, remove and insert again so that the item
        	// 		  is put in the right place.
        	heap.remove(item);
        	heap.add(item);
        }
        
        //aravi5: If the size of the queue goes beyond, 
        //remove the element with least count from queue.
    ...
    /**
     * aravi5: The priority queue is not serialized and deserialized and hence
     * cannot be run on distributed systems.
     **/
...
//aravi5: SketchComparator class to specify how to compare elements in the Priority Queue.
//		  Compare the elements based on their frequency value stored in sketch.
class SketchComparator<T> implements Serializable, Comparator<T> {
	...
	public int compare(T s1, T s2) {
		return (int)(sketch.estimateCount((String)s1) - sketch.estimateCount((String)s2));
	}
}
